...
- **Exact same-native-session reincarnation substrate (cond-0378 B1)** — a versioned, append-only immutable restore-contract record bound to the exact M3-A roster source (stable agent, native lineage, source incarnation), carrying canonical cwd/trust-root, harness/provider and bounded route provenance, exact executable identity, profile/provider-home references-and-digests, observed model/effort, execution mode, and typed unavailable/missing facts (never secret values, canonical real paths only); deterministic canonical payload/digest with exact-replay adopt/conflict/append semantics, full stored-record decode + digest verification before the atomic live→dormant roster transition, a conditional-write/CAS that serializes concurrent dormant transitions, and caller-owned transaction boundaries for both publication and the transition.
- **Legacy native-identity migration and exact-live-pane resolution (cond-0377D)** — adds a read-only live legacy audit, explicit one-candidate migration with intent-first at-most-once `/status` observation and committed-evidence/journal adoption on exact retry (never resend), versioned provider capability cells that are green only with committed-chain receipts and an observed exact executable (Kimi remains unresolved without a session and no synthetic turn), and `GET /roster/pane-identity` for bounded exact-live-pane → terminal → unique LIVE incarnation → agent/lineage resolution, the `conduct whoami` fork primitive. `CAO_LEGACY_MIGRATION_PRODUCER_ENABLED=0` disables only new migration operations; retained rows remain queryable/adoptable.
- **Muse managed-v2 native enrollment (cond-0377B)** — `muse_cli` joins the native-TUI launchable providers with a discovery-first launch path: a fresh no-prompt TUI (`muse --trust-workspace --yolo --reasoning-effort <e> --model <id>` — no `resume`, no `--session-id`, no prompt, no task bytes) generates the provider session id itself, and the managed launch types `/status` once and parses the provider-generated canonical UUID at zero turns as the pre-task identity/readiness proof naming the exact session/model/effort/profile/provider/cwd and idle zero-turn state. The native attachment for that exact id is claimed only after the status observation succeeds (acquisition `provider_status_discovered`, never chosen-session); the durable terminal id, bind receipts, and roster binding all carry the discovered id, and any discovery/publish/persistence failure attempts the exact pane teardown — with any cleanup failure surfaced in the typed preflight detail — and never advertises readiness. `muse resume <known-id>` is retained strictly as the restoration form for a later reincarnation slice. The CAO profile system prompt is composed into the session as base instructions through the installed `TBH_EVAL_APPEND_SYSTEM_PROMPT_FILE` surface (deterministically verified on the installed 0.1.0-R708.1 build on launch and exact restore; `--agents <JSON>` does not compose into the main session on this build and is not used for the role/profile). Admission delivers the task once through a Muse queue operation scoped to managed-v2 admission; the generic `/control-input` surface stays closed for Muse. Capability payload and bind receipts name `muse-native-status-idle` / `provider_status_discovered` / executable `muse` pinned to `0.1.0`
