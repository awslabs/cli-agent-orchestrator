...
- Fix Kimi CLI supervisor handoff always timing out because Kimi CLI's default MCP tool call timeout is 60s (`tool_call_timeout_ms=60000` in `kimi_cli/config.py`): the `handoff` MCP tool creates a worker terminal, waits for completion, and extracts output, which routinely exceeds 60s; add `--config mcp.client.tool_call_timeout_ms=600000` to the kimi launch command when MCP servers are configured, matching the 600s handoff timeout in `_handoff_impl()` and consistent with the Codex provider's `tool_timeout_sec=600.0` fix
