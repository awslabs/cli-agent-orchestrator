...
- assign: return-fast via server-side deferred provider init. `POST /sessions/{name}/terminals` now accepts `defer_init=true` (with an optional `initial_message` + `initial_message_orchestration_type`); the endpoint returns as soon as the tmux window and DB row exist, and runs `provider.initialize()` plus initial-message delivery as an asyncio background task. `_assign_impl` uses this path so the assign() MCP tool returns in <2s regardless of provider init duration. Fixes two related problems: (a) kiro-cli 2.11's ~60s per-tool client timeout previously cancelled assign RPCs when CAO's blocking round-trip took long enough to push past that cap, forcing the LLM to fall back to `send_message` for delivery; (b) two concurrent assigns from the same LLM turn no longer serialize on `_assign_impl`'s blocking `requests.post` — three concurrent assigns from a real kiro-cli 2.11 supervisor now dispatch in ~0.85s wall-clock (was ~92s) with all three workers' `provider.initialize()` running truly in parallel
