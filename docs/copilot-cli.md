...
- `--additional-mcp-config @<runtime-json>`

Implementation:

- Add `cao-mcp-server` with `CAO_TERMINAL_ID`
- Write secure temp config using `tempfile.mkstemp()`
- Remove file in `cleanup()`
...
When `--additional-mcp-config` is missing from your `copilot --help`, CAO skips MCP config injection and logs a warning.

### Command Shape

```bash
copilot --allow-all [--agent <name>] --config-dir ~/.copilot \
  --add-dir <cwd> --additional-mcp-config @<runtime-mcp.json> --autopilot
```
