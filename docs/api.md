...
- `use_worktree` (bool, optional, default `false`, issue #100 Phase 1): When `true`, provision an isolated `git worktree` for this terminal instead of sharing `working_directory` as given. The worktree is created on its own branch from the resolved repository's current `HEAD`, at `<repo_root>/.cao/worktrees/<terminal_id>`, and removed automatically (directory + branch) when the terminal is deleted. Requires the resolved `working_directory` (explicit, or the caller's inherited cwd when omitted) to be inside a git repository — otherwise the request fails with `400 Bad Request`.
