...
| `workflow_journal_capture_output` | `false` | ⚠️ **Security-relevant opt-in.** Whether the durable workflow run journal retains step **output text**. Off by default: the journal stores execution metadata only (events, timings, states, error kinds, and a non-revealing `sha256:` content reference), never prompt text or step output. Turning this ON means step output payloads land in the journal and are exported by the run diagnostics bundle. Retained text is size-capped (below) and cleaned through the shared `audit_log` sanitizer — which is transport hygiene (control-character stripping, size limiting), **not** secret redaction: a credential in a step's output is retained verbatim. Prefer leaving this off; the payload-bearing run read routes are scope-gated, not redacted. |
