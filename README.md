...
* **Persistent agent memory** – Agents store and recall knowledge across sessions using `memory_store` and `memory_recall` MCP tools. CAO automatically injects relevant memories as context at session start, with auto-save hooks for Claude Code and Kiro CLI. Non-git projects drop an opt-in `.cao/project_id` marker so memories survive folder rename/`mv` — add `.cao/` to `.gitignore` if you don't want to check it in. See [docs/memory.md](docs/memory.md).
