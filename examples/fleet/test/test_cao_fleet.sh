#!/usr/bin/env bash
# test/test_cao_fleet.sh — assertions for cao-fleet node->endpoint resolution
# and the exec passthrough. Hermetic: builds a temp registry and stubs `cao`, so
# it needs no real fleet.json, no cao install, and no network.
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CAO_FLEET="$HERE/../bin/cao-fleet"
fail=0
...
assert_eq "$("$CAO_FLEET" show node-a)" "http://100.64.0.11:9889" "show node-a (default port)"
assert_eq "$("$CAO_FLEET" show node-b)" "http://100.64.0.12:9889" "show node-b (default port)"
assert_eq "$("$CAO_FLEET" show node-d)" "http://100.64.0.14:9999" "show node-d (per-node port override)"

# --- unknown node exits non-zero ---
"$CAO_FLEET" show nope >/dev/null 2>&1
assert_eq "$?" "1" "unknown node exits 1"

# --- list prints one row per node ---
count="$("$CAO_FLEET" list | wc -l | tr -d ' ')"
assert_eq "$count" "4" "list prints 4 rows"
assert_eq "$("$CAO_FLEET" list | grep -c '^node-d 100.64.0.14$')" "1" "list row content for node-d"
...
STUBSH
chmod +x "$STUB/cao"
out="$(PATH="$STUB:$PATH" "$CAO_FLEET" exec node-d session list)"
assert_eq "$out" "HOST=100.64.0.14 PORT=9999 ARGS=session list" "exec exports host/port + forwards args"

# --- missing registry exits 2 ---
CAO_FLEET_CONFIG="$TMP/does-not-exist.json" "$CAO_FLEET" list >/dev/null 2>&1
