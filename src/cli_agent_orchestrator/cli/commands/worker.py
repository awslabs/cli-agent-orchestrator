...
def status(worker_id, as_json):
    ...
    if lease is None or lease.get("state") in LIVE_STATES:
        try:
            terminal = client.terminal(worker_id, client.sole_terminal(worker_id)["id"])
            last_output = client.terminal_output(worker_id, terminal["id"])
        except click.ClickException as exc:
            # A live lease with no answering cao-server is the first minute of
            # every worker's life — the pod is pulling its image or installing
            # its profile. The lease is already in hand, so degrade to it
            # rather than replacing the answer with a connection error. No
            # lease AND no terminal is different: nothing was found, and the
            # error is the answer.
            if lease is None:
                raise
