"""Admission and control for a managed Codex native-TUI session.

This is a provider-owned adapter, parallel to the Claude and Kimi adapters.
The journal is provider-private so a control receipt from either sibling
cannot satisfy a Codex operation.
...
* **Transport truth is not provider truth.** ``posted`` says bytes were
  written. Whether Codex accepted them is a separate observation
...
#: Codex's canonical provider and executable key are both ``codex``.
PROVIDER = "codex"

#: The executable name, which is what the version-pin tables are keyed by.
PROVIDER_EXECUTABLE = provider_contracts.PROVIDER_CODEX

RECORD_SCHEMA = "cao-codex-native-control-v1"
INTENT_SCHEMA = "cao-codex-native-control-intent-v1"
TURN_OBSERVATION_SCHEMA = "cao-codex-native-turn-observation-v1"
PROVIDER_OBSERVATION_SCHEMA = "cao-codex-native-control-observation-v1"
KEYSTROKE_PLAN_SCHEMA = "cao-codex-native-keystroke-plan-v1"
...
#: Codex installs via ``/terminal-setup``, so whether they exist depends
...
_PROVEN_COMPOSER_NEWLINE: dict[str, dict[str, Any]] = {
    "0.146.0": {
        ...
        "evidence": {
            "launcher_sha256": ("134063e133f0b4244fa3b251acf973d4fe4b4aeeacbdc135211bf480f59f1477"),
            "composer_binding": "EditorKeymap.insert_newline includes Ctrl-J",
...
class NativeControlError(RuntimeError):
    """Base class for every Codex native control failure."""
...
def plan_composer_keystrokes(
    ...
    *,
    ...
    trailing whitespace is invariant under trimming, so whether Codex
    ...
    # "codex-cli 0.146.0" -- because that is what the provider printed,
...
def _validated_turn_observation(observation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if observation.get("schema") != TURN_OBSERVATION_SCHEMA:
        raise NativeControlInvalid(
            ...
            f"Codex observation, and neither is an unlabelled dict"
        )
...
def _fetch(db: Any, operation_id: str) -> Any:
    return (
        db.query(database.CodexNativeControlOperationModel)
        .filter(database.CodexNativeControlOperationModel.operation_id == operation_id)
        .first()
    )
...
def _validate_binding(
    *,
    ...
    if binding["execution_mode"] != NATIVE_TUI:
        raise NativeControlInvalid(
            f"codex native control operates only in {NATIVE_TUI!r}; "
    ...
    *,
    ...
    try:
        with database.SessionLocal() as db:
            ...
            db.add(database.CodexNativeControlOperationModel(**row_values))
            db.commit()
    ...
    try:
        with database.SessionLocal() as db:
            ...
            updated = (
                db.query(database.CodexNativeControlOperationModel)
                .filter(
                    database.CodexNativeControlOperationModel.operation_id == operation_id,
                    database.CodexNativeControlOperationModel.epoch == observed_epoch,
                )
...
def submit_composer_plan(
    *,
    ...
    """Submit Codex text already typed from ``plan``.

    The generic control-input path observes the payload resting in Codex's
    composer before it crosses the submit boundary.  Keeping the boundary in
    this provider adapter lets both the fused and observed paths use the same
    pinned settle and exactly one Enter.
    """
    # Let the composer leave its documented paste-burst suppression window
    # before the explicit submit.
    ...
    try:
        with database.SessionLocal() as db:
            row = (
                db.query(database.CodexNativeControlOperationModel)
                .filter(
                    database.CodexNativeControlOperationModel.native_session_id
                    == native_session_id,
                    database.CodexNativeControlOperationModel.state == AMBIGUOUS,
                )
                .order_by(database.CodexNativeControlOperationModel.created_at)
                .first()
            )
            return None if row is None else _row_dict(row)
    except Exception as exc:  # noqa: BLE001 - fail closed
