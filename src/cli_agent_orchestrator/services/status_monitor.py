...
class StatusMonitor:
    ...
    def __init__(self):
        ...
        # pyte ``Screen`` + ``Stream`` — typed ``Any`` because pyte ships no
        # type stubs and is imported lazily on the screen-detection path.
    ...
    def _detect_screen(self, terminal_id: str, provider) -> TerminalStatus:
        ...
        try:
            status: TerminalStatus = provider.get_status_from_screen(lines)
            return status
        except Exception:
