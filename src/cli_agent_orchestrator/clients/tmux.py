...
class TmuxClient:
    ...
    def create_session(
        ...
        try:
            ...
            #
            # Keep the subprocess-based session layer from the psmux branch,
            # but carry forward the upstream detached-pane sizing fix by using
            # tmux/psmux CLI flags directly.
            cmd = [
                "tmux", "new-session",
                "-s", session_name,
                "-n", window_name,
