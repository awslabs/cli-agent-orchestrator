...
class TmuxLookupError(RuntimeError):
    """A tmux session lookup could not be ANSWERED (not: answered "absent").

    Raised only by ``TmuxClient.session_exists_strict``, whose whole purpose is
    to keep "the session is gone" separable from "the query failed". Teardown
    turns this into a failed, re-runnable teardown with the registry left intact
    (#498), which is the fail-CLOSED direction.
    """
...
class TmuxClient:
    ...
    def kill_session(self, session_name: str) -> bool:
        ...
        transient tmux/socket error during the verification poll must NOT be
        misread as "session gone". The strict check raises ``TmuxLookupError`` on
        an unanswerable lookup, which propagates to the ``except`` below and
        returns False — so a kill is never reported as confirmed while the
        ...
        try:
            ...
            if session:
                ...
                while True:
                    if not self.session_exists_strict(session_name):
    ...
    def session_exists(self, session_name: str) -> bool:
        ...
        Lenient: any lookup error collapses to False ("assume absent"). Fine for
        best-effort callers (status/UI), but NOT for teardown confirmation, where
        "couldn't tell" must be distinguished from "gone" — use
        ``session_exists_strict`` there.
        """
        try:
            session = self.server.sessions.get(session_name=session_name)
