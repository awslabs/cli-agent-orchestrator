...
BUSY_PATTERN = (
    r"\b(thinking|working|running|executing|processing|analyzing|modifying|applying edits)\b"
)
...
class CopilotCliProvider(BaseProvider):
    ...
    def _command(self) -> str:
        ...
        command_parts.extend(["--add-dir", os.getcwd()])
    ...
    def _accept_trust_prompts(self, timeout: float = 30.0) -> None:
        ...
        while time.time() - start < timeout:
            ...
            if re.search(r"\[\s*y/n\s*]", content):
                self._send_key("y")
                self._send_enter()
                time.sleep(1)
                continue
    ...
    def initialize(self) -> bool:
        ...
        while time.time() < deadline:
            ...
            if status in (TerminalStatus.IDLE, TerminalStatus.COMPLETED):
                # Copilot can surface trust prompts a few seconds after first
                # showing an idle prompt. Require a stable ready streak before
