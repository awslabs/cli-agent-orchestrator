...
class MacroNotationParseBody(BaseModel):
    notation: Optional[str] = None
...
async def parse_macro_notation_endpoint(
    body: MacroNotationParseBody,
    _scopes: List[str] = Depends(require_any_scope(SCOPE_READ, SCOPE_WRITE, SCOPE_ADMIN)),
) -> Dict[str, Any]:
    """The §5.3 server-authoritative parse: ``{events, preview}`` or 422.

    Read-scoped like the list route: parsing changes no state.  The client's
    live preview uses the same pinned grammar; this endpoint is the
    authority that decides what may be saved or sent.
    """
    ...
    if body.notation is None:
        return JSONResponse(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content={"errors": [{"offset": None, "message": "'notation' is required"}]},
        )
    try:
        events, preview = await asyncio.to_thread(parse_with_preview, body.notation)
        ...
        return JSONResponse(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content={"errors": [{"offset": None, "message": str(exc)}]},
        )
    return {"events": events, "preview": preview}
