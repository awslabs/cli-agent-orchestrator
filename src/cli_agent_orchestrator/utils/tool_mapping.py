...
TOOL_MAPPING: Dict[str, Dict[str, List[str]]] = {
    ...
    # Qwen Code (qwen) is a Gemini-CLI fork and shares the same gemini-style tool
    # vocabulary (write_file/read_file/run_shell_command/...). Restrictions are
    # enforced softly via the injected security prompt (see
    # SOFT_ENFORCEMENT_PROVIDERS); this mapping only drives the launch-time
