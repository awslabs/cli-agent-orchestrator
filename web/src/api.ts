...
/**
 * Build the graph query-string fragment from optional scope filters.
 * Centralizes the `scope`/`scope_id` encoding so `getGraph` and `exportGraph`
 * cannot drift.
 */
function buildGraphQueryString(scope?: string, scopeId?: string): string {
  const params = [
...
export const api = {
  ...
  deleteSession: (name: string) =>
    fetchJSON<{ success: boolean; deleted: string[]; errors: any[] }>(
      `/sessions/${name}`,
      { method: "DELETE" },
    ),
  ...
  getGraph: (provider = "memory", scope?: string, scopeId?: string) => {
    return fetchJSON<GraphView>(
      `/graph/${encodeURIComponent(provider)}${buildGraphQueryString(scope, scopeId)}`,
      { timeoutMs: 120000 },
    );
  },
  exportGraph: (
    ...
    return fetchJSON<GraphExportResult>(
      `/graph/${encodeURIComponent(provider)}/export${buildGraphQueryString(scope, scopeId)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
