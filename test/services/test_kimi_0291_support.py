...
1.  every version gate (contract, route, bridge) accepts 0.29.1 and still
    accepts 0.29.0, and rejects everything else — an exact set, never a
    range;
2.  a multi-line message on a 0.29.1 session is deliverable, because the
    composer-newline table carries a proven 0.29.1 entry whose keystroke
    plan is byte-identical to 0.29.0's; and
3.  a receipt records the *actual* installed version, so a 0.29.1 binary
    is never described by a 0.29.0 constant (or the reverse).
"""
...
# --------------------------------------------------------------------
# The version gates accept exactly {0.29.1, 0.29.0}
# --------------------------------------------------------------------


def test_the_accepted_set_is_current_then_retained():
    assert pc.SUPPORTED_VERSIONS["kimi"] == (CURRENT, RETAINED)
    assert pc.PINNED_VERSIONS["kimi"] == CURRENT
    assert kimi_route.SUPPORTED_KIMI_VERSIONS == (CURRENT, RETAINED)


@pytest.mark.parametrize("banner", [f"kimi {CURRENT}", f"kimi {RETAINED}", CURRENT, RETAINED])
def test_check_pinned_version_accepts_both_builds(banner):
    pc.check_pinned_version("kimi", banner)


@pytest.mark.parametrize("bad", ["0.29.2", "0.28.9", "0.30.0", "1.0.0", "kimi", ""])
...
# --------------------------------------------------------------------
# A multi-line message is deliverable on a proven 0.29.1 session
# --------------------------------------------------------------------


def test_the_composer_newline_table_has_a_separate_proven_0291_entry():
    ...
    assert entry["keystroke"] == knc._PROVEN_COMPOSER_NEWLINE[RETAINED]["keystroke"]
...
@pytest.mark.parametrize("version", [CURRENT, RETAINED])
def test_a_multiline_message_is_deliverable_on_both_builds(version):
