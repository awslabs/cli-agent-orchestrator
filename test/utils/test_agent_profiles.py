...
class TestParseAgentProfileTextHtmlCommentStrip:
    """Regression tests for HTML-comment stripping before frontmatter parsing (03e6d35)."""

    def test_leading_html_comment_stripped_before_frontmatter(self):
        """Profile with a leading <!-- ... --> comment parses correctly."""
        from cli_agent_orchestrator.utils.agent_profiles import parse_agent_profile_text

        text = "<!-- generated by AIM -->\n---\nname: my-agent\ndescription: Test\n---\nPrompt"
    ...
    def test_no_comment_passthrough(self):
        """Profile without a leading comment parses identically (strip is a no-op)."""
        from cli_agent_orchestrator.utils.agent_profiles import parse_agent_profile_text

        text = "---\nname: plain\ndescription: Plain\n---\nPlain prompt"
        profile = parse_agent_profile_text(text, "plain")

        assert profile.name == "plain"
        assert profile.description == "Plain"
        assert profile.system_prompt == "Plain prompt"

    def test_multiline_html_comment_stripped(self):
        """Multiline <!-- ... --> comment is stripped (re.DOTALL behavior)."""
        from cli_agent_orchestrator.utils.agent_profiles import parse_agent_profile_text

        text = "<!--\ngenerated\nby AIM\n-->\n---\nname: multi\ndescription: Multi\n---\nBody"
        profile = parse_agent_profile_text(text, "multi")

        assert profile.name == "multi"
        assert profile.description == "Multi"
        assert profile.system_prompt == "Body"

    def test_non_leading_comment_not_stripped(self):
        """Comment that is NOT at the start of text should not be stripped."""
        from cli_agent_orchestrator.utils.agent_profiles import parse_agent_profile_text

        # The text starts with frontmatter, comment appears in the body — should remain.
        text = "---\nname: keep\ndescription: Keep\n---\n<!-- keep this -->\nBody"
