...
class TestRefreshInstalledAgentForProfile:
    ...
    def test_returns_empty_list_when_no_installed_json_exists(self, tmp_path, monkeypatch):
...
class TestRefreshAllCaoManagedAgents:
    ...
    def test_refreshes_only_cao_managed_subset_across_kiro_and_q_dirs(self, tmp_path, monkeypatch):
        context_dir = tmp_path / "agent-context"
        kiro_dir = tmp_path / "kiro"
        q_dir = tmp_path / "q"
        managed_kiro_context = context_dir / "managed-kiro.md"
        managed_q_context = context_dir / "managed-q.md"
        managed_kiro_context.parent.mkdir(parents=True, exist_ok=True)
        managed_kiro_context.write_text("kiro", encoding="utf-8")
        managed_q_context.write_text("q", encoding="utf-8")
        ...
        _write_json(
            managed_kiro,
            {
                ...
                "resources": [f"file://{managed_kiro_context}"],
            },
        )
        _write_json(
            unmanaged_kiro,
        ...
        _write_json(
            managed_q,
            {
                ...
                "resources": [f"file://{managed_q_context}"],
            },
        )
        _write_json(
            unmanaged_q,
        ...
        assert refreshed == [managed_kiro, managed_q]
