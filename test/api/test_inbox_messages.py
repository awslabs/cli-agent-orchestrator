...
class TestGetInboxMessagesEndpoint:
    ...
    def test_get_all_messages_success(self, client, sample_inbox_messages):
        ...
        with patch("cli_agent_orchestrator.api.main.get_inbox_messages") as mock_get:
            ...
            response = client.get("/terminals/a1b2c3d4/inbox/messages")
    ...
    def test_get_messages_with_status_filter(self, client, sample_inbox_messages):
        ...
        with patch("cli_agent_orchestrator.api.main.get_inbox_messages") as mock_get:
            ...
            response = client.get("/terminals/a1b2c3d4/inbox/messages?status=pending")
    ...
    def test_get_messages_with_limit(self, client, sample_inbox_messages):
        ...
        with patch("cli_agent_orchestrator.api.main.get_inbox_messages") as mock_get:
            ...
            response = client.get("/terminals/a1b2c3d4/inbox/messages?limit=2")

            assert response.status_code == 200
            data = response.json()
            assert len(data) == 2
            mock_get.assert_called_once_with("a1b2c3d4", limit=2, status=None)
    ...
    def test_get_messages_with_status_and_limit(self, client, sample_inbox_messages):
        ...
        with patch("cli_agent_orchestrator.api.main.get_inbox_messages") as mock_get:
            ...
            response = client.get("/terminals/a1b2c3d4/inbox/messages?status=pending&limit=5")

            assert response.status_code == 200
            data = response.json()
            assert len(data) == 1
            mock_get.assert_called_once_with("a1b2c3d4", limit=5, status=MessageStatus.PENDING)

    def test_invalid_status_parameter(self, client):
        """Test error handling for invalid status parameter."""
        response = client.get("/terminals/a1b2c3d4/inbox/messages?status=invalid_status")
    ...
    def test_all_status_values(self, client, sample_inbox_messages):
        ...
        for status_value in ["pending", "delivered", "failed"]:
            ...
            with patch("cli_agent_orchestrator.api.main.get_inbox_messages") as mock_get:
                ...
                response = client.get(f"/terminals/a1b2c3d4/inbox/messages?status={status_value}")
