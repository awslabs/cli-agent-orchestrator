...
def test_create_terminal_uses_explicit_provider_override(mock_get, mock_post):
    ...
    mock_post_response.json.return_value = {"id": "term1234"}
    ...
    assert mock_post.call_args.kwargs["params"]["provider"] == ProviderType.COPILOT.value


@patch("cli_agent_orchestrator.mcp_server.server.requests.post")
@patch("cli_agent_orchestrator.mcp_server.server.requests.get")
@patch("cli_agent_orchestrator.mcp_server.server.load_agent_profile")
def test_create_terminal_uses_profile_provider_when_no_override(
    mock_load_profile, mock_get, mock_post
):
    ...
    mock_post_response = MagicMock()
    mock_post_response.raise_for_status.return_value = None
    mock_post_response.json.return_value = {"id": "term5678"}
    mock_post.return_value = mock_post_response
    ...
    assert mock_post.call_args.kwargs["params"]["provider"] == ProviderType.OPENCODE.value
...
def test_create_terminal_falls_back_to_default_provider_in_new_session(
    mock_load_profile, mock_gen_session, mock_post
):
    mock_load_profile.side_effect = RuntimeError("Profile not found")

    mock_post_response = MagicMock()
    mock_post_response.raise_for_status.return_value = None
    mock_post_response.json.return_value = {"id": "term9012"}
    mock_post.return_value = mock_post_response
    ...
    assert mock_post.call_args.kwargs["params"]["provider"] == DEFAULT_PROVIDER
