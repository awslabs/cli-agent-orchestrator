...
class TestStatus:
    ...
    def test_a_booting_worker_still_shows_its_lease(self, runner, broker):
        """The first minute of every worker's life must not read as an error.

        The lease is already in hand when the terminal fetch fails, so `status`
        degrades to it — this is the moment `status` exists for, and a bare
        connection error would hide the one fact it had.
        """
    ...
    def test_the_booting_hint_is_not_printed_twice(self, runner, broker):
        """`sole_terminal`'s own message already sends the reader to the log."""
        broker.workers.return_value = [_lease("w1", "leased")]
        broker.sole_terminal.side_effect = click.ClickException(
    ...
    def test_no_lease_and_no_terminal_is_an_error_not_a_shrug(self, runner, broker):
        """A mistyped id must not exit 0 with two 'unknown' rows."""
        broker.workers.return_value = []
