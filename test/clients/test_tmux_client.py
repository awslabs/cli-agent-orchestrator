...
class TestGetPaneCurrentCommand:
    @patch("cli_agent_orchestrator.clients.tmux.subprocess")
    def test_get_pane_current_command_success(self, mock_sp, tmux):
        mock_sp.run.side_effect = [
            _completed("", returncode=0),            # has-session
            _completed("win\n", returncode=0),      # list-windows
            _completed("bash\n", returncode=0),     # display-message
    ...
    @patch("cli_agent_orchestrator.clients.tmux.subprocess")
    def test_get_pane_current_command_session_not_found(self, mock_sp, tmux):
        mock_sp.run.return_value = _completed("", returncode=1)
    ...
    @patch("cli_agent_orchestrator.clients.tmux.subprocess")
    def test_get_pane_current_command_window_not_found(self, mock_sp, tmux):
        mock_sp.run.side_effect = [
            _completed("", returncode=0),            # has-session
            _completed("other\n", returncode=0),    # list-windows
        ]
    ...
    @patch("cli_agent_orchestrator.clients.tmux.subprocess")
    def test_get_pane_current_command_exception_returns_none(self, mock_sp, tmux):
        mock_sp.run.side_effect = Exception("tmux error")
