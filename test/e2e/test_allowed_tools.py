...
# ---------------------------------------------------------------------------
# Qwen Code provider — soft enforcement via SECURITY_PROMPT
# ---------------------------------------------------------------------------


@pytest.mark.e2e
class TestQwenCliAllowedTools:
    """E2E allowedTools tests for the Qwen Code provider.

    Qwen Code launches with ``--approval-mode yolo`` and does not enforce a
    native tool denylist for CAO; restrictions are enforced softly via the
    SECURITY_PROMPT appended to the system prompt. This is advisory only.
    """

    @pytest.mark.xfail(
        reason="Qwen Code lacks native CAO tool blocking under yolo; soft enforcement is advisory only",
        strict=False,
    )
    def test_restricted_supervisor_cannot_bash(self, require_qwen_cli):
        """Supervisor with only @cao-mcp-server should not execute bash."""
        _run_restricted_tool_test(
            provider="qwen_cli",
            agent_profile="code_supervisor",
    ...
    def test_unrestricted_developer_can_bash(self, require_qwen_cli):
        """Developer with wildcard allowedTools can execute bash."""
        _run_unrestricted_tool_test(
            provider="qwen_cli",
            agent_profile="developer",
    ...
    def test_allowed_tools_stored_in_metadata(self, require_qwen_cli):
        """allowed_tools is persisted and returned by GET /terminals."""
        _run_allowed_tools_stored_test(
            provider="qwen_cli",
            agent_profile="developer",
